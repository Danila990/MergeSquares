...
namespace LargeNumbers
{
    ...
    public struct AlphabeticNotation : IEquatable<AlphabeticNotation>
    {
        ...
        [NonSerialized]
        private static Dictionary<EColor, string> _colors = new Dictionary<EColor, string>()
            {
                [EColor.None] = String.Empty,
                [EColor.Yellow] = "<color=#FFD925>"
            };
...
#pragma warning restore CA1031 // Do not catch general exception types
        ...
        public string ToColorString(EColor color, EStringPart stringPart = EStringPart.Magnitude)
        {
            sb.Clear();

            if (stringPart == EStringPart.Coefficient || stringPart == EStringPart.Both)
                sb.Append(_colors[EColor.Yellow]);

            var testMagnitude = magnitude;
            if (testMagnitude < 0) testMagnitude = -testMagnitude;
            if (testMagnitude == 0)
                return sb.Append(coefficient).ToString();

            if (testMagnitude == 1 && coefficient < 1)
                return sb.Append(coefficient * 1000).ToString();


            sb.ConvertAndAppendTruncated(coefficient);
            if (stringPart == EStringPart.Magnitude || stringPart == EStringPart.Both)
                sb.Append(_colors[color]);
            sb.Append(GetAlphabeticMagnitudeName(magnitude));
            return sb.ToString();
        }
        
        public string ToStringNaturalNumber()
        {
            string result;
            ...
            if ( testMagnitude == 0 )
            {
                result = Math.Round(coefficient).ToString();
                return result;
            }

            if ( testMagnitude == 1 && coefficient < 1 )
            {
                result = (coefficient * 1000).ToString();
                return result;
            }
        ...
        public string ToColorStringNaturalNumbers(EColor color, EStringPart stringPart = EStringPart.Magnitude)
        {
            sb.Clear();
            ...
            if ( testMagnitude < 0 ) testMagnitude = -testMagnitude;
            if ( testMagnitude == 0 )
                return sb.Append(Math.Round(coefficient)).ToString();
        ...
        public StringConstructor ConstructString() => 
            new StringConstructor(coefficient, magnitude);

        public static AlphabeticNotation zero => new AlphabeticNotation ( );
        public static AlphabeticNotation one => new AlphabeticNotation (1);
        ...
        public static implicit operator float(AlphabeticNotation number)
        {
            if (number.magnitude == 0)
                return Convert.ToSingle(number.coefficient);

            if (number.coefficient == 0)
                return 0f;

            return Convert.ToSingle(Math.Pow(1000, number.magnitude) * number.coefficient);
        }

        public static explicit operator AlphabeticNotation(double number)
        {
            return new AlphabeticNotation(number);
        }

        public static explicit operator AlphabeticNotation(float number)
        {
            return new AlphabeticNotation(number);
        }
        ...
        public struct StringConstructor
        {
            ...
            private static int _magnitude;

            private static bool _naturalNumber;
            private static Dictionary<EStringPart, EColor> _colorsOfParts = new Dictionary<EStringPart, EColor>(2)
            {
                [EStringPart.Coefficient] = EColor.None,
                [EStringPart.Magnitude] = EColor.None
            };

            public StringConstructor(double coefficient, int magnitude)
            {
                _coefficient = coefficient;
                _magnitude = magnitude;
                sb.Clear();
                _naturalNumber = false;
                _colorsOfParts[EStringPart.Coefficient] = _colorsOfParts[EStringPart.Magnitude] = EColor.None;
            }

            public override string ToString()
            {
                sb.Append(_colors[_colorsOfParts[EStringPart.Coefficient]]);

                var testMagnitude = _magnitude;
                if (testMagnitude < 0) testMagnitude = -testMagnitude;
                if (testMagnitude == 0)
                {
                    _coefficient = _naturalNumber ? Math.Round(_coefficient) : _coefficient;
                    return sb.Append(_coefficient).ToString();
                }

                if (testMagnitude == 1 && _coefficient < 1)
                    return sb.Append(_coefficient * 1000).ToString();

                sb.ConvertAndAppendTruncated(_coefficient);
                sb.Append(_colors[_colorsOfParts[EStringPart.Magnitude]]);
                sb.Append(GetAlphabeticMagnitudeName(_magnitude));
                
                return sb.ToString();
            }
            
            public StringConstructor SetColorForPart(EStringPart stringPart, EColor color)
            {
                switch (stringPart)
                {
                    case EStringPart.None:
                        return this;
                    case EStringPart.Both:
                        _colorsOfParts[EStringPart.Coefficient] = _colorsOfParts[EStringPart.Magnitude] = color;
                        return this;
                }
                
                _colorsOfParts[stringPart] = color;
                return this;
            }

            public StringConstructor AsNaturalNumber()
            {
                _naturalNumber = true;
                return this;
            }
